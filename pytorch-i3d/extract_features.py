...
from charades_dataset_lwj import Charades as Dataset
        ...
        with torch.no_grad():
            for data in dataloaders[phase]:
                ...
                if t > 1600:
                    ...
                    if (features == None):
                        np.save(os.path.join(save_dir, name[0]),feature)
                    else:
                        feature=features.squeeze(0).permute(1,2,3,0).data.cpu().numpy()
...
if __name__ == '__main__':
    ...
    for i in range(1,101):
        if (i==37 or i==99):
            continue
        print(i)
        path="D:/code/Graphics/crawler/Gandata/shots/{}/".format(i)
        run(mode="rgb", root=path, load_model='models/rgb_charades.pt', save_dir=path,split=path+"mydataset.json")
